...
**The rebuild guard is per-CLIENT, not global — do not "simplify" it back into a module global.** It lives in the `render-state` `dcc.Store`, so each browser tracks what it was last sent. When it was a module global, the first browser to poll consumed each tick and stamped it as built, and every other viewer got `no_update` for the figure forever: measured 0% figure responses for a second client while the first got 95%. That is *exactly* the "status bar live, chart frozen" symptom, and it needs two connected browsers to reproduce — a single-client test will never show it. View state (`_view_mode`, anchors) stays global on purpose; "have I already been sent this figure?" is per-browser and must not.

Second factor worth knowing: the callback is lock-serialized (`_callback_lock`) and one figure build costs ~100 ms, so **every extra open browser tab divides the refresh rate**. Measured with two clients: the second gets roughly a tenth of the first's request rate. Close the host browser when watching remotely. (The lock currently spans the whole callback including the figure build; narrowing it to just the bookkeeping would remove most of the contention, but that means snapshotting view state for the build — untried so far.)
